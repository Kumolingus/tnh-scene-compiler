...
- **Allowlists**: two-layer (base + project), loaded in `allowlists.py`, validated in `validator.py`. **Anything that reports what a writer may type must read both layers** — a project's own allowlists may legitimately hold only what it adds, with the game's values coming from `allowlists_base`. `tnh_generate_cheatsheet` gets this via `loader.load_layered()`; the mod repo's `Tools/generate_test_scenes` takes a list of layer directories for the same reason. A new single-layer consumer would silently drop the whole base game.
