...
- **Glossary**: `glossary.py` — a non-modal reference window (editor toolbar "Glossary" button) with a searchable section list + copyable code examples, parsed at runtime from the bundled `docs/glossary/*.md` files (one per top-level section, numbered for order; read in name order, concatenated, then parsed). Pure parser (`parse_glossary`) testable without Tkinter; the glossary folder is in the `.spec` datas.
