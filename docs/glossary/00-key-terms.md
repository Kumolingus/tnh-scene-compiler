...
suggestion) instead of breaking the game. You don't edit the allowlist
yourself; if you need a value that isn't in it, ask the developer to add it.
...
themselves from the game state (see below); the other types are opened by
the mod's own code.

### How a cinematic scene gets played

A cinematic scene registers itself as an event. At the moment named by its
